...
const BorderMagicButton = ({
    ...
    return (
        <button className="relative w-full inline-flex h-12 overflow-hidden rounded-lg p-[1px] focus:outline-none md:w-60 md:mt-10">
            ...
            <span className={`inline-flex h-full w-full cursor-pointer items-center justify-center rounded-lg bg-slate-950 px-7 text-sm font-medium text-white backdrop-blur-3xl ${classes}`}>
                {position === 'left' && icon}
                {title}
                {position === 'right' && icon}
            </span>
